...
func connect_to_noray():
	...
	if address.contains(":"):
		...
		err = await Noray.connect_to_host(hostee, port)
	else:
...
func _handle_connect(address: String, port: int) -> Error:
	...
	if role == Role.CLIENT:
		...
		if err != OK and err != ERR_BUSY:
			print("Handshake failed: %s" % error_string(err))
			return err
		elif err == ERR_BUSY:
			print("Partial handshake, proceeding with connection.")
		else:
